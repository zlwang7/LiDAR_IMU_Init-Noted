...
// States needed by Initialization  初始化所需的状态
struct CalibState {
    M3D rot_end;
    V3D ang_vel;
    V3D linear_vel;
...
struct Angular_Vel_Cost_only_Rot {
    ...
    bool operator()(const T *q, T *residual) const {
        ...
        Eigen::Matrix<T, 3, 1> resi = R_LI * Lidar_ang_vel_T - IMU_ang_vel_T;     // 残差= R*雷达角速度 - IMU角速度
        residual[0] = resi[0];
        residual[1] = resi[1];
        residual[2] = resi[2];
        return true;
...
struct Angular_Vel_Cost {
    ...
    bool operator()(const T *q, const T *b_g, const T *t, T *residual) const {
        ...
        Eigen::Matrix<T, 3, 1> resi =
                R_LI * Lidar_ang_vel_T - IMU_ang_vel_T - (deltaT_LI_T + td) * IMU_ang_acc_T + bias_g;  // 对应论文公式(18)
        residual[0] = resi[0];
        residual[1] = resi[1];
        residual[2] = resi[2];
        return true;
...
struct Linear_acc_Cost {
    ...
    bool operator()(const T *q, const T *b_a, const T *trans, T *residual) const {
        ...
        Eigen::Matrix<T, 3, 1> resi = R_LL0_T * R_LI_T_transpose * IMU_linear_acc_T - R_LL0_T * bias_aL
                                      + R_GL0 * STD_GRAV - Lidar_linear_acc_T - R_LL0_T * Jacob_trans_T * T_IL;  // 对应论文公式(23)
