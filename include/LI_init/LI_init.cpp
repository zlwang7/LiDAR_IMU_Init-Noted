...
void LI_Init::downsample_interpolate_IMU(const double &move_start_time) {
    ...
    while (IMU_state_group_ALL.front().timeStamp < move_start_time - 3.0)     //IMU_state_group_ALL 在 push_ALL_IMU_CalibState 函数
    ...
    //Mean filter to attenuate noise         进行均值滤波  mean_filt_size是选取近邻数据的范围
    ...
    //Down-sample and interpolation，Fig.4 in the paper       fig.4 下采样
    for (int i = 0; i < Lidar_state_group.size(); i++) {
        for (int j = 1; j < IMU_state_group_ALL.size(); j++) {
            if (IMU_state_group_ALL[j - 1].timeStamp <= Lidar_state_group[i].timeStamp
                && IMU_state_group_ALL[j].timeStamp > Lidar_state_group[i].timeStamp) {          //如果相邻两帧IMU刚好跨过一帧lidar
                ...
                double s = delta_t_right / delta_t;   //  位于lidar时间戳右边的dt / 总dt
                IMU_state_interpolation.ang_vel = s * IMU_state_group_ALL[j - 1].ang_vel +               //插值
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (j < 0 || j > N - 1)
                ...
                corr += (IMU_state_group[i].ang_vel.norm() - mean_IMU_ang_vel) *
                        (Lidar_state_group[j].ang_vel.norm() - mean_LiDAR_ang_vel);  // Zero-centered cross correlation  对应公式13
            }
        }
...
void LI_Init::IMU_time_compensate(const double &lag_time, const bool &is_discard) {
    if (is_discard) {  // 放弃前10个激光雷达估计和相应的IMU测量
...
void LI_Init::cut_sequence_tail() {
    ...
    //Align the size of two sequences      //平衡IMU和LIDAR的size
    while (IMU_state_group.size() > Lidar_state_group.size())
        IMU_state_group.pop_back();
    while (IMU_state_group.size() < Lidar_state_group.size())
        Lidar_state_group.pop_back();
...
void LI_Init::solve_Rotation_only() {
    ...
    ceres::LocalParameterization *quatParam = new ceres::QuaternionParameterization();  //创建ceres
    ...
    for (int i = 0; i < IMU_state_group.size(); i++) {
        ...
        Lidar_angvel_skew << SKEW_SYM_MATRX(Lidar_state_group[i].ang_vel);  //没用上
...
void LI_Init::solve_Rot_bias_gyro(double &timediff_imu_wrt_lidar) {
    ...
    for (int i = 0; i < IMU_state_group.size(); i++) {
        double deltaT = Lidar_state_group[i].timeStamp - IMU_state_group[i].timeStamp;   //当前帧lidar和imu的时间差
    ...
    IMU_time_compensate(get_lag_time_2(), false);  // IMU时间补偿 根据最新计算的time_lag_2
...
bool LI_Init::data_sufficiency_assess(MatrixXd &Jacobian_rot, int &frame_num, V3D &lidar_omg, int &orig_odom_freq,
    ...
    M3D lidar_omg_skew;
    lidar_omg_skew << SKEW_SYM_MATRX(lidar_omg);    // w^
    Jacobian_rot.block<3, 3>(3 * frame_num, 0) = lidar_omg_skew;   //构建雅可比矩阵Jr
    ...
    if (frame_num % orig_odom_freq * cut_frame_num == 0) {
        ...
        V3D EigenValue = es.eigenvalues().real();         //特征值
        M3D EigenVec_mat = es.eigenvectors().real();       //特征向量

        M3D EigenMatCwise = EigenVec_mat.cwiseProduct(EigenVec_mat);   //特征向量的平方
        ...
        int maxPos[3] = {0};
        maxPos[0] = max_element(EigenMat_1_col.begin(), EigenMat_1_col.end()) - EigenMat_1_col.begin();     //第1列特征向量最大值所在位置
        maxPos[1] = max_element(EigenMat_2_col.begin(), EigenMat_2_col.end()) - EigenMat_2_col.begin();     //第2列特征向量最大值所在位置
        maxPos[2] = max_element(EigenMat_3_col.begin(), EigenMat_3_col.end()) - EigenMat_3_col.begin();     //第3列特征向量最大值所在位置

        V3D Scaled_Eigen = EigenValue / data_accum_length;   //the larger data_accum_length is, the more data is needed  特征值/300
        ...
        int axis[3];
        axis[2] = max_element(maxPos, maxPos + 3) - maxPos;     //maxPos最大值所在位置
        axis[0] = min_element(maxPos, maxPos + 3) - maxPos;     //maxPos最小值所在位置
...
void LI_Init::LI_Initialization(int &orig_odom_freq, int &cut_frame_num, double &timediff_imu_wrt_lidar,
    ...
    downsample_interpolate_IMU(move_start_time);   //论文fig.4 下采样   move_start_time是雷达开始移动的时间戳   
    fout_before_filter();
    IMU_time_compensate(0.0, true);  // IMU时间补偿  初始为0  但置成true是为了放弃前10个激光雷达估计和相应的IMU测量
    ...
    cut_sequence_tail();    //去掉尾部

    xcorr_temporal_init(orig_odom_freq * cut_frame_num);
    IMU_time_compensate(get_lag_time_1(), false);   // IMU时间补偿 根据最新计算的time_lag_1
    ...
    solve_Rotation_only();  //先只求解rot   以这个rot作为下一步初值

    solve_Rot_bias_gyro(timediff_imu_wrt_lidar);  //再次求解rot  并求解bias

    acc_interpolate();  //加速度插值

    solve_trans_biasacc_grav();  //求解trans  bias  grav
...
void LI_Init::print_initialization_result(double &time_L_I, M3D &R_L_I, V3D &p_L_I, V3D &bias_g, V3D &bias_a, V3D gravity) {
    cout.setf(ios::fixed);
